...
package object framework {
  ...
  implicit def arbMatrix[S: ClassTag : Gen]: Arbitrary[Mat[S]] = arbMatrixOfN[S](100, 100)

  /** Yield an arbitrary Vector of 100 values. */
  implicit def arbVector[S: ClassTag : Gen]: Arbitrary[Vec[S]] = arbVectorOfN[S](100)

  /** Yield an arbitrary Series of 100 values. */
  implicit def arbSeries[X: Gen : Ordering : ClassTag, S: ClassTag : Gen]: Arbitrary[Series[X, S]] = arbSeriesOfN[X, S](100)

  /** Yield an arbitrary 10 x 100 Frame. */
  implicit def arbFrame[X: Gen : Ordering : ClassTag, S: ClassTag : Gen]: Arbitrary[Frame[X, Int, S]] = arbFrameOfN[X, S](10, 100)
  ...
  def genSeriesOfN[X: Gen : Ordering : ScalarTag, S: Gen : ScalarTag](size: Int): Gen[Series[X, S]] =
    for {
  ...
  def genFrameOfN[X: Gen : Ordering : ScalarTag, S: Gen : ScalarTag](dsize: Int, ssize: Int): Gen[Frame[X, Int, S]] =
    Gen.listOfN(dsize, genSeriesOfN[X, S](ssize)).map(ss => Frame(ss: _*))

  /** Yield an arbitrary 100 x 100 Matrix. */
  def arbMatrixOfN[S: ClassTag : Gen](cols: Int, rows: Int): Arbitrary[Mat[S]] = Arbitrary(genMatrixOfN[S](cols, rows))

  /** Yield an arbitrary Vector of 100 values. */
  def arbVectorOfN[S: ClassTag : Gen](size: Int): Arbitrary[Vec[S]] = Arbitrary(genVectorOfN[S](size))

  /** Yield an arbitrary Series of 100 values. */
  def arbSeriesOfN[X: Gen : Ordering : ClassTag, S: ClassTag : Gen](size: Int): Arbitrary[Series[X, S]] = Arbitrary(genSeriesOfN[X, S](size))

  /** Yield an arbitrary 100 x 100 Series. */
  def arbFrameOfN[X: Gen : Ordering : ClassTag, S: ClassTag : Gen](cols: Int, rows: Int): Arbitrary[Frame[X, Int, S]] = Arbitrary(genFrameOfN[X, S](cols, rows))
}
